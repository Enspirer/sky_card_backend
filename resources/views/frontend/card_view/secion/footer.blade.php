                ...
                @if(count($portfolio) != 0)
                    @foreach($portfolio as $web_link)
                            @if($web_link->content_type == 'Website Links')
                                '<div class="row m-0"style="font-size: 13px" > <p>{{$web_link->caption}} : &nbsp;</p> <a href="{{$web_link->website_links}}" target="_blank">{{$web_link->website_links}}</a> </div>'+
                            @endif
                    @endforeach
                @endif
            '');
                ...
                @if(count($portfolio) != 0)
                    @foreach($portfolio as $web_link_m)
                            @if($web_link_m->content_type == 'Website Links')
                                '<p>{{$web_link_m->caption}} : <span><a href="{{$web_link_m->website_links}}" target="_blank">{{$web_link_m->website_links}}</span></p> </a><br>' +
                            @endif
                    @endforeach
                @endif
            '');
